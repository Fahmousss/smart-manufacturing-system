...
type MachineType = 'CNC' | 'Milling' | 'Press' | 'Assembly';
...
export default function Edit({ machine, operators }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        name: machine.name,
        type: machine.type,
        mqtt_topic_id: machine.mqtt_topic_id,
        current_operator_id: machine.current_operator?.id?.toString() || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(MachineController.update(machine.id).url);
    };
